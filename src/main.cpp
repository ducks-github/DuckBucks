...
#if defined(NDEBUG)
# error "Duckbucks cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9a185a959b6a91cebbf9f893060c6ac5e2cac3b25fe62cc1d2225a90120501a3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Duckbucks: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Duckbucks Signed Message:\n";
...
{
    // Duckbucks: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 52 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 694200); // Duckbucks: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // Duckbucks: 2.5 days
static const int64 nTargetSpacing = 3.14158 * 60; // Duckbucks: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Duckbucks: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Duckbucks: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x86b1c546631e818024240459daf1740ce030a703eb2ec3231c21f2c8f2b7f06e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " lol eat bread, also texas is the last free place on earth ;)- duck  8/3/23";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 52 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0413e97c8d1dcdafdd460a10f77aad81db53a19d1077e5676f753e4bf405653a721b2dbcc6f2bbedea87cb7d34073b4a597f9b600cc7fcbe733a6580286a33736e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1691057235;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084714370;

        if (fTestNet)
        {
            block.nTime    = 1691057000;
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x86b1c546631e818024240459daf1740ce030a703eb2ec3231c21f2c8f2b7f06e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc3, 0xb6, 0xde }; // Duckbucks: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DuckbucksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DuckbucksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DuckbucksMiner(CWallet *pwallet)
{
    printf("DuckbucksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("duckbucks-miner");
    ...
    try { loop {
        ...
        printf("Running DuckbucksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DuckbucksMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DuckbucksMiner, pwallet));
}
